...
from ..helper.import_institutes_ranking_data import InstituteRankingExcelToJsonParser, update_raking
from ..utils import DataframeUtil
from .base import BaseTask


class ImportInstitutesRankingFromExcelTask(BaseTask):
    name = "ImportInstitutesRankingFromExcelTask"
    ...
    def run(self, filepath, *args, **kwargs):
        ...
        parser = InstituteRankingExcelToJsonParser(dataframe)
        ...
        if not status:
            ...
            for data in rows_data:
                ...
                status, errors = update_raking(data, row)
                if not status:
                    result = errors
                    break
                if row < total_record:
...
@celery_app.task(bind=True, base=ImportInstitutesRankingFromExcelTask)
def import_institutes_raking_task(self, *args, **kwargs):
