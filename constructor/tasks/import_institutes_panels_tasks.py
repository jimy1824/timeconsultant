...
from ..helper.import_institutes_panels_data import InstitutePanelExcelToJsonParser, update_panel
from ..utils import DataframeUtil
from .base import BaseTask


class ImportInstitutesPanelFromExcelTask(BaseTask):
    name = "ImportInstitutesPanelFromExcelTask"
    ...
    def run(self, filepath, *args, **kwargs):
        ...
        parser = InstitutePanelExcelToJsonParser(dataframe)
        ...
        if not status:
            ...
            for data in rows_data:
                ...
                status, errors = update_panel(data, row)
                if not status:
                    result = errors
                    break
                if row < total_record:
...
@celery_app.task(bind=True, base=ImportInstitutesPanelFromExcelTask)
def import_institutes_panels_task(self, *args, **kwargs):
