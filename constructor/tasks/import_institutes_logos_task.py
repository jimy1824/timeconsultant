...
from ..helper.import_institute_logos import InstituteLogoExcelToJsonParser, update_logo
from ..utils import DataframeUtil
from .base import BaseTask


class ImportInstitutesLogosFromExcelTask(BaseTask):
    name = "ImportInstitutesLogosFromExcelTask"
    ...
    def run(self, filepath, *args, **kwargs):
        ...
        parser = InstituteLogoExcelToJsonParser(dataframe)
        ...
        if not status:
            ...
            for data in rows_data:
                ...
                status, errors = update_logo(data, row)
                if not status:
                    result = errors
                    break
                if row < total_record:
...
@celery_app.task(bind=True, base=ImportInstitutesLogosFromExcelTask)
def import_institutes_logo_task(self, *args, **kwargs):
